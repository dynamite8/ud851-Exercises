...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        Toast error = Toast.makeText(getContext(), "Invalid scale factor entered.  Try a number between 0 and 3.", Toast.LENGTH_LONG);

        if (preference instanceof EditTextPreference && newValue instanceof String) {
            try {
                float value = Float.valueOf((String)newValue);
                boolean isValid = value >=0 && value <= 3;
                if (!isValid) {
                    error.show();
                }
                return isValid;
