...
public class SettingsFragment extends PreferenceFragmentCompat implements OnSharedPreferenceChangeListener {
    ...
    public void onCreatePreferences(Bundle bundle, String s) {
        ...
        PreferenceScreen screenPreference = getPreferenceScreen();
        SharedPreferences sharedPreferences = screenPreference.getSharedPreferences();
        for (int i = 0; i < screenPreference.getPreferenceCount(); i++) {
            Preference preference = screenPreference.getPreference(i);
    ...
    private void setPreferenceSummary(Preference preference, String value) {
        if (preference instanceof ListPreference) {
            ...
            int indexSelected = list.findIndexOfValue(value);
            if (indexSelected >= 0) {
                // get all the labels with getEntries() and choose the one that user has selected
                list.setSummary(list.getEntries()[indexSelected]);
            }
        }
    }
